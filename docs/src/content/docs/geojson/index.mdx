...
[foreign members](https://tools.ietf.org/html/rfc7946#section-6.1) — extra keys that are not part of
the RFC 7946 schema. These are stored on every `GeoJsonObject` as `foreignMembers: JsonObject`. They
are **not** `Feature.properties`, and they are not interpreted as GeoJSON types (the RFC example
`centerline` looks like a LineString but is not one).

A common case is GOFS `zone_id`, a string sibling of `properties` on a Feature:
...
On encode, foreign members are flattened as sibling JSON keys. A `"foreignMembers"` key is never
emitted. Use `decodeForeignMembers<T>()` to decode the bag into a `@Serializable` type,
`toForeignMembers()` to build a bag from one, or set `foreignMembers` on any GeoJSON DSL builder.
